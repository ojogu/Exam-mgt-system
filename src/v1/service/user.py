...
from src.v1.service.courses import LevelService, DeptService, CourseService
...
class UserService():
    ...
    async def authenticate_user(self, user_data:Login):
        try:
            ...
            if not user:
                ...
                logger.warning(f"Authentication failed: User with identifier '{identifier}' does not exist.")
    ...
    async def fetch_all_students(self):
        try:
            ...
            logger.error(f"An unexpected error occurred while fetching all students: {e}")
            raise ServerError()
    ...
    async def check_if_user_exist_by_id(self, id:uuid.UUID):
        try:
            ...
            logger.error(f"Error checking if user exists by id {id}: {e}", exc_info=True)
            raise ServerError()
    ...
    async def link_lecturer_to_course(self, user_data:LectCourse ):
        try:
            ...
            user = await self.check_if_user_exist_by_id(user_data.lecturer_id)
            if not user:
                logger.warning(f"User {user_data.lecturer_id} not found.")
                raise NotFoundError()
            if user.role != Role_Enum.LECTURER:
                logger.warning(f"User {user.id} is not a lecturer.")
                raise AuthorizationError(f"{user.id} does not have permission to link to course")
            ...
            if course.department.id != user.department.id:
                logger.warning(f"Course {course.code} and user {user.id} are not in the same department.")
            ...
            self.db.add(user)
            await self.db.commit()
            logger.info(f"Successfully linked lecturer {user.first_name} to course {course.name}.")
            return True
        except SQLAlchemyError as e:
            logger.error(f"Database error while linking lecturer {user_data.lecturer_id} to course {user_data.course_id}: {e}", exc_info=True)
            await self.db.rollback()
            raise ServerError()
        # except Exception as e:
